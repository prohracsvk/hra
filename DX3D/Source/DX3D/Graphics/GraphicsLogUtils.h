	...
	namespace GraphicsLogUtils
	{
		inline void CheckShaderCompile(Logger& logger, HRESULT hr, ID3DBlob* errorBlob)
		{
			...
			if (FAILED(hr))
			{
				...
				{
					DX3DLogThrow(logger, std::runtime_error, Logger::LogLevel::Error, "Shader/ShaderBinary compilation failed")
				}
			
			}
			if (errorMsg)
			{
				DX3DLog(logger, Logger::LogLevel::Warning, "Warning For Shaders");
			}
		}
